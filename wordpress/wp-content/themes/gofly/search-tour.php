...
function search_posts_by_title_only($search_term, $post_type = 'tour', $limit = -1)
{
    global $wpdb;
    ...
    $args = array(
        ...
        'orderby'        => 'date',
        'order'          => 'DESC',
        'suppress_filters' => false, // ensure filters work
    );

    // Add temporary WHERE filter
    add_filter('posts_where', function ($where) use ($wpdb, $search_term) {
        // Restrict to post_title only
        return $where . " AND {$wpdb->posts}.post_title LIKE '%{$search_term}%'";
    });

    // Run the query
    $query = new WP_Query($args);

    // Remove the filter immediately after use to prevent side effects
...
<div class="package-grid-page pt-100 mb-100">
    <div class="container">
        <div class="row g-4">
            ...
            $query = search_posts_by_title_only($search_term, 'tour', 10);
            if ($query->have_posts()) {
            ...
            ?>
                <div class="pagination-area wow animate fadeInUp mt-60" id="hide-ax" data-wow-delay="200ms" data-wow-duration="1500ms">
