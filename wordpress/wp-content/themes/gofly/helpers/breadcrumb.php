...
if (!function_exists('egns_breadcrumb')) {
    ...
    function egns_breadcrumb($list_style = 'ul', $list_id = 'breadcrumb', $list_class = 'breadcrumb-list', $active_class = 'active', $echo = true)
    {
        ...
        // Front page
        if (is_front_page()) {
            $breadcrumb .= '<li class="' . $active_class . '">' . esc_html__('Home', 'gofly') . ' </li>';
        } else {
            $breadcrumb .= '<li class="breadcrumb-item"><a href="' . esc_url(home_url()) . '">' . esc_html__('Home', 'gofly') . ' </a></li>';
        }
        ...
        // Search
        if (is_search()) {
            $breadcrumb .= '<li class="' . $active_class . '">' . get_search_query() . '</li>';
        }

        // 404
        if (is_404()) {
            $breadcrumb .= '<li class="' . $active_class . '">' . esc_html__('404', 'gofly') . '</li>';
        }
        ...
        if (is_single() && get_post_type() != 'post' && get_post_type() != 'attachment') {
            ...
            if ($cpt_obj) {
                ...
                if (is_post_type_hierarchical($cpt_obj->name)) {
                    ...
                    if (!empty($cpt_taxes) && is_taxonomy_hierarchical($cpt_taxes[0])) {
                        $cpt_terms = get_the_terms(get_the_ID(), $cpt_taxes[0]);

                        if (is_array($cpt_terms) && !empty($cpt_terms)) {
                            $term = $cpt_terms[0];
                            $breadcrumb .= '<li><a href="' . esc_url(get_term_link($term)) . '">' . esc_html($term->name) . '</a></li>';
                        }
                    }
                }
                $breadcrumb .= '<li class="' . $active_class . '">' . esc_html(get_the_title()) . '</li>';
        ...
        // Ouput
        if ($echo) {
            echo sprintf(esc_html__("%s", 'gofly'), $breadcrumb);;
        } else {
            return $breadcrumb;
        }
    }
