...
<?php if (Egns\Helper\Egns_Helper::is_enabled($enable_breadcrumb_by_theme, $breadcrumb_enable_by_page)): ?>
    <?php
    // Color from page option
    $breadcrumb_page_bg_color = Egns_Helper::egns_page_option_value('breadcrumb_page_bg_color');
    // Images
    ...
    // If page color is selected, don't load image
    if (!empty($breadcrumb_page_bg_color)) {
        $inline_style = 'style="background-color: ' . esc_attr($breadcrumb_page_bg_color) . ';"';
    } else {
